...
class GamesVoucherController extends ApiController
{
  ...
  public function listProductVoucher()
  {
    $dataProduct = $this->listProductByCategory('VG');
    $result = $dataProduct;
    return $this->sendResponse(0, "Sukses", $result);
  }

  public function listProductByCategory($params)
  {
    $select = [
      'games_item.title',
      'games_item.game_code',
      'games.code',
      'games.category_code'
    ];
    $filterProduct = GamesItem::select($select)
      ->join('games', 'games_item.game_code', '=', 'games.code')
      ->where('games.category_code', '=', $params)
      ->get();
